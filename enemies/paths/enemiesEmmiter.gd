...
export(bool) var enemis_stick_target = true
...
func _process(delta):
	if shape != null:
		...
		var target_pts = get_shape_pts(get_shape_polygons(), enemies.size())
		for i in range(enemies.size()):
			if enemies[i] != null:
				enemies[i].target = target_pts[i]


func emit_enemies(enemies_nb, total_duration=0): # todo: + intervales
...
func remove_bullet(ref):
	if enemies != null and ref in enemies:
		enemies.erase(ref)